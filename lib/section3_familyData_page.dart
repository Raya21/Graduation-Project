...
class _Section3State extends State<Section3> {
  ...
  Map<String, TextEditingController> _RegistrationNumber = {};
  Map<String, TextEditingController> _CollegeName = {};
  Map<String, TextEditingController> _UniversityName = {};
  _brothersDataControllers() {
    for (int i = 0; i < 2000; i++) {
      _StudentName.addAll({"$i $uId": TextEditingController()});
      _RegistrationNumber.addAll({"$i $uId": TextEditingController()});
      _CollegeName.addAll({"$i $uId": TextEditingController()});
      _UniversityName.addAll({"$i $uId": TextEditingController()});
    }
  }
    ...
    if (count2 != Null) {
      ...
      setState(() {
        for (int bId = 0; bId < count2!; bId++) {
          ...
          _RegistrationNumber['$bId $uId']!.text =
              response[bId]["RegistrationNumber"].toString();
          _CollegeName['$bId $uId']!.text = response[bId]["CollegeName"];
          _UniversityName['$bId $uId']!.text = response[bId]["UniversityName"];

          _onUpdate(
              bId.toString(),
              response[bId]["StudentName"],
              response[bId]["RegistrationNumber"].toString(),
              response[bId]["CollegeName"],
              response[bId]["UniversityName"]);
        }
      });
    }
  }
  ...
  Future save() async {
    ...
    var url = "http://192.168.1.10/handinhand2/familydatas3.php";
      ...
      child: Column(
          ...
          children: [
              ...
              child: TextFormField(
                ...
                onChanged: (val) {
                  ...
                  _onUpdate(
                      key,
                      StudentName,
                      _RegistrationNumber['$key $uId']!.text,
                      _CollegeName['$key $uId']!.text,
                      _UniversityName['$key $uId']!.text);
                },
                cursorColor: Colors.purple,
            ...
            SizedBox(
              width: 300,
              child: TextFormField(
                controller: _RegistrationNumber['$key $uId'],
                onChanged: (val) {
                  RegistrationNumber = val;
                  _onUpdate(
                      key,
                      _StudentName['$key $uId']!.text,
                      RegistrationNumber,
                      _CollegeName['$key $uId']!.text,
                      _UniversityName['$key $uId']!.text);
                },
            ...
            SizedBox(
              width: 300,
              child: TextFormField(
                controller: _CollegeName['$key $uId'],
                onChanged: (val) {
                  CollegeName = val;
                  _onUpdate(
                      key,
            ...
            SizedBox(
              width: 300,
              child: TextFormField(
                controller: _UniversityName['$key $uId'],
                onChanged: (val) {
                  UniversityName = val;
                  _onUpdate(
                      key,
